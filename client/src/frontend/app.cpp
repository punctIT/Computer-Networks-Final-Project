...
App::App()
{
    ...
    alert_popup = std::make_shared<AlertPopup>(window.get(),page_manager);
    ...
    QObject::connect(data_requster.get(), &DataRequester::UpdateAlersPopup, window.get(), [this](QString mesaj) {
        ...
        try{
            ...
            qDebug()<<last_alert_id;
        }
        catch(const std::exception& e){
            qDebug()<<"invaid laast_id";
            return;
        }
        auto content = data.value()["content"].value();
        auto data_alert = BetterString::split(content,"[]");
        if(data_alert.size()>=5)
            alert_popup->showMessage(data_alert[1].c_str(),data_alert[5].c_str(), true);
    });
