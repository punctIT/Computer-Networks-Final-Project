...
UnknownSyslogDashboardScreen::UnknownSyslogDashboardScreen(std::shared_ptr<DataRequester> data, const std::shared_ptr<PageManager> &page_manager, std::shared_ptr<QMainWindow> window) : Page(data, page_manager, window)
{
    ...
    SyslogDashboard_table= std::make_shared<UnknownSyslogTable>();
...
UnknownSyslogTable::UnknownSyslogTable()
{
    widget = new QWidget();
...
QWidget *UnknownSyslogTable::get_widget()
{
    return this->widget;
}

void UnknownSyslogTable::clear()
{
    table->setRowCount(0); 
}
void UnknownSyslogTable::add(std::vector<std::string> whitelist_data)
{
